...
public class Dilation2D implements Layer {
    ...
    public Dilation2D(Array kernel, Activation activation, Padding padding) {
        this.kernel = kernel;
        this.activation = activation;
        this.padding = padding;
    }

    public Dilation2D(Array kernel, Activation activation) {
        this.kernel = kernel;
        this.activation = activation;
    }

    public Dilation2D(Array kernel) {
        this.kernel = kernel;
    }
    ...
    public Array execute(Array input) {
        ...
        input = padding.equals(Padding.SAME)? Function.padding(input, kernelShape, Double.NEGATIVE_INFINITY): input;
            ...
            for (int j = 0; j < (inputShape[1] - kernelShape[0] + 1); j++) {
                ...
                for (int k = 0; k < (inputShape[2] - kernelShape[1] + 1); k++) { 
                    ...
                    for (int l = 0; l < kernelShape[3]; l++) {
                        ...
                        doubles = ArrayUtils.add(doubles, Arrays.stream(Function.dilation(window, morphKernel)).reduce(0.0, (r, d) -> r + d));
                    }
                }
            }
        }
